  ...
  public async extractAndCacheImageFrames(imageRow: ImageRow): Promise<ImageFrameMeta[]> {
    ...
    try {
      ...
      for (let i = 0; i < frames.length; i++) {
        // Use the same `<filename>-...` naming convention as crops/resizes so
        // that `deleteImagesFromStorage` cleans frame files up automatically.
        const frameFilename = `${imageRow.filename}-frame-${i}.png`
        const framePath = `${config.dir.CROP_DIR}/${frameFilename}`
        await fs.writeFile(framePath, frames[i].buffer)
        meta.push({ filename: frameFilename, delay_ms: frames[i].delay })
      }
